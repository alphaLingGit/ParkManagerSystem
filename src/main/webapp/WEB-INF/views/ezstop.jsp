...
<script>
    ...
    function showUsers(result) {
        ...
        $.each(result.extend.cards.list,function (index,item) {
            ...
            let $idnum = $("<td></td>").append(dateFtt("yyyy-MM-dd hh:mm:ss",item.startTime));
            let $carNum = $("<td></td>").append(dateFtt("yyyy-MM-dd hh:mm:ss",item.leaveTime));
            let $balance = $("<td></td>").append(item.parkingTime);
            let $isInPosition = $("<td></td>").append(item.isInPosition);
            let $operation = $("<td></td>")
...
//                        .append($("<button class='btn btn-danger deletebtn'delete-id="+item.userId+">Delete</button>"));
            $("<tr></tr>").append($userId).append($username).append($idnum)
                .append($carNum).append($balance).append($isInPosition)
//                .append($operation)
